...
function Header() {
  ...
  const cartDilog = () => {
    ...
    document.addEventListener("click", handleClickOutside, true);
    dialog[0].attributes[0].ownerElement.hidden =
      !dialog[0].attributes[0].ownerElement.hidden;
  };
  const handleClickOutside = (value) => {
    let dialog = document.getElementsByClassName("cart-dialog");
    if (
      value.target.classList != "cart-dialog" &&
      value.target.offsetParent.classList != "cart-dialog"
    )
      ...
      <header>
        {Close ? (
          <div className="offermessage">
            <div className="messages">
              Shop For AED 199 & Get Extra 10% Off*. Code: EXTRA10.*T&C{" "}
            </div>

            <label
              class="close-header-top-ads cursorP mb-0"
              onClick={onClose}
            ></label>
          </div>
        ) : (
        ...
        <div className="header-container">
          ...
          <nav className="nav-items">
            ...
            {/* to={'/track-order'} */}
            <Link class="nav-link nav-hyber-link">
